...
async def evaluate_pipeline(queries, relevance_dict, vs_manager, llm, top_k=5):
    ...
    for q in tqdm(queries, desc="Evaluating"):
        ...
        rel_scores_orig = {
            doc_id: (3 if doc_id in relevant_docs else 0) for doc_id in retrieved_orig
        }
        ...
        metrics_rewritten["MRR"].append(
            reciprocal_rank(relevant_docs, retrieved_rewritten)
        )
        metrics_rewritten["Recall@10"].append(
            recall_at_k(relevant_docs, retrieved_rewritten, 10)
        )
        rel_scores_rewritten = {
            doc_id: (3 if doc_id in relevant_docs else 0)
            for doc_id in retrieved_rewritten
        }
        metrics_rewritten["nDCG@10"].append(
            ndcg_at_k(relevant_docs, rel_scores_rewritten, 10)
        )
        ...
        metrics_expanded["nDCG@AllExpanded"].append(
            ndcg_at_k(relevant_docs, rel_scores_exp, len(retrieved_exp))
        )
...
async def main():
    ...
    results = await evaluate_pipeline(queries, relevance_dict, vs_manager, llm=llm)
